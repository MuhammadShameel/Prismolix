...
export const fetchAllBlogs = async () => {
  try {
    const response = await fetch(
      "https://api.prismolix.com/wp-json/wp/v2/portfolio?_embed&per_page=10",
      {
...
export const fetchBlogBySlug = async (slug) => {
  try {
    const response = await fetch(
      `https://api.prismolix.com/wp-json/wp/v2/portfolio?slug=${slug}&_embed`,
      {
        next: { revalidate: 3600 }, // Cache for 1 hour
      }
    );
...
export const extractBlogData = (portfolio) => {
  ...
  const servicesProvided =
    acf.services_provided || "UI/UX Design, Dev (Headless WordPress)";
  // Challenge description (supports alternate ACF keys)
  const portfolioChallengesRaw =
    acf.portfolio_challanges || acf.portfolio_challenges || null;
    ...
    if (typeof portfolioChallengesRaw === "string") {
      ...
      challengePoints = portfolioChallengesRaw
        .map((item) =>
          typeof item === "string"
            ? item
            : item?.point || item?.title || item?.text
        )
        .filter(Boolean);
    } else if (typeof portfolioChallengesRaw === "object") {
      // Try common shapes
      challenge =
        portfolioChallengesRaw.description ||
        portfolioChallengesRaw.text ||
        challenge;
      const pts =
      ...
      if (Array.isArray(pts)) {
        challengePoints = pts
          .map((p) =>
            typeof p === "string" ? p : p?.point || p?.title || p?.text
          )
          .filter(Boolean);
      }
    }
  ...
  if (!challenge) {
    challenge =
      "Agency struggling to keep up with demand — high-volume landing pages, inconsistent freelance help, and slipping deadlines";
  }
  ...
  const gallery = Array.isArray(acf.gallery)
    ? acf.gallery.map((item) => {
        ...
        const url =
          item?.url || item?.sizes?.medium_large || item?.sizes?.large;
