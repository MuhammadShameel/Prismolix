...
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation, Autoplay, Pagination } from "swiper/modules";
import { fetchBlogBySlug, extractBlogData } from "../../../services/blog.service.js";
    ...
    <section className="lg:px-5 md:px-4 px-3 my-lg">
      <div className="container mx-auto">
        <div className="testimonial-bg relative flex flex-col md:flex-row p-4 justify-center">
          ...
          <div className="flex flex-col items-center justify-center gap-5 max-w-6xl my-lg">
            <span className="font-medium text-[26px]  color-primary-dark">[Case Study]</span>
            <h1 className="text-center leading-[95%]">
            ...
            <p className="text-center">
              {loading ? "" : (
                <span
                  dangerouslySetInnerHTML={{ __html: data?.excerpt || data?.content || "" }}
                />
              )}
            </p>
          </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex gap-1.5 mt-5 flex-wrap">
            {loading ? (
              <span className="bg-light-purple px-5 py-2.5 rounded-full">Loading...</span>
            ) : data?.tags?.length ? (
              data.tags.map((tag, i) => (
                <span key={i} className="bg-light-purple px-5 py-2.5 rounded-full">
                  {tag}
                </span>
              ))
            ) : (
              <span className="bg-light-purple px-5 py-2.5 rounded-full">Case Study</span>
            )}
          </div>
          ...
          <div className="bg-light-purple flex lg:flex-row flex-col lg:gap-7 gap-4 px-10 py-6 rounded-[20px]">
            <div className="lg:w-3/12 w-12/12">
              <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px]">Category</span>
              <p>{loading ? "—" : data?.industry || data?.categories?.[0] || "—"}</p>
            </div>
            <div className="border-r-1 bg-black opacity-20"></div>
            <div className="lg:w-3/12 w-12/12">
              <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px]">Time Taken</span>
              <p>{loading ? "—" : diffInMonths(data?.startDate, data?.endDate)}</p>
            </div>
            <div className="border-r-1 bg-black opacity-20"></div>
            <div className="lg:w-3/12 w-12/12">
              <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px]">Start Date</span>
              <p>{loading ? "—" : formatMonthYear(data?.startDate)}</p>
            </div>
            <div className="border-r-1 bg-black opacity-20"></div>
            <div className="lg:w-3/12 w-12/12">
              <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px]">Completed Date</span>
          ...
          <div className="bg-light-purple flex lg:flex-row flex-col lg:items-center px-10 py-6 rounded-[20px] lg:gap-10 gap-3">
            <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px] ">Technologies Used</span>
            <ul className="flex lg:gap-10 gap-2.5">
              {(data?.technologiesUsed && data.technologiesUsed.length > 0
                ? data.technologiesUsed
                : [{ imageUrl: null }]
              ).map((tech, idx) => (
                <li key={idx} className="border-black/25 border p-4 rounded-full">
                  {tech.imageUrl ? (
                    <Image src={tech.imageUrl} alt={tech.name || "Tech"} width={20} height={20} />
                  ) : (
          ...
          <div className="bg-light-purple flex flex-col px-10 py-6 rounded-[20px] gap-12">
            <span className="font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px] ">Methods Used</span>
            <div className="flex lg:flex-row flex-col lg:gap-[30px] gap-[20px]">
              {(data?.methodsUsed?.length ? data.methodsUsed : ["—"]).map((method, idx) => (
                <div key={idx} className="lg:w-4/12 w-12/12">
                  <p className="font-medium w-full text-center py-2.5 scroll-px-24 rounded-full bg-[#C4B5FD] ">{method}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex lg:flex-row flex-col gap-5">
            <div className="bg-light-purple lg:w-1/2 w-2/2 px-10 py-6 rounded-[20px] flex flex-col gap-2.5">
              <h4>The Challenge</h4>
              {/* <p className="font-medium">{loading ? "" : data?.challenge || "—"}</p>
              {data?.challengePoints?.length ? (
                <ul className="flex flex-col">
                  {data.challengePoints.map((pt, idx) => (
                    <li key={idx} className="flex items-center gap-2.5 font-medium lg:text-[26px] md:text-[24px] sm:text-[20px] text-[18px] ">
                 ...
                 <p className="font-medium">{loading ? "" : data?.description || "—"}</p>
              <ul className="flex flex-col">
      ...
      <div className="container mx-auto my-lg">
        <div className="service-content flex flex-col items-center gap-5">
          <span className="font-medium text-[26px] color-primary-dark">[The Process]</span>
        ...
        <div className="service-cards grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-10">
          {(data?.process?.length ? data.process : []).map((step) => (
            <div key={step.number + step.title} className="relative flex flex-col bg-light-purple rounded-[30px] p-8 overflow-hidden">
      ...
      <div className="container mx-auto my-lg">
        <div className="service-content flex flex-col items-center gap-5">
          <span className="font-medium text-[26px] color-primary-dark">[Tools]</span>
          <h2 className="leading-20 tracking-tight max-w-2xl text-center">Tools & Platforms We Use</h2>
        </div>
      </div>
    </section>
  );
